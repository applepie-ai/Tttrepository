        ...
        for (i_out_x = 0; i_out_x < dim_im_out_x; i_out_x++)
        {
                 ...
                 i_ker_y++)
            {
                     ...
                     i_ker_x++)
                {
                    ...
                    {
                        arm_q7_to_q7_reordered_no_shift((q7_t *) Im_in + (i_ker_y * dim_im_in_x + i_ker_x) * ch_im_in,
                                                         pBuffer, ch_im_in);
                    }
                    pBuffer += ch_im_in;
                }
            ...
            {
                pOut =
                    arm_nn_mat_mult_kernel_q7_reordered(wt, (q7_t *)bufferA, ch_im_out, ch_im_in * dim_kernel_x * dim_kernel_y,
    ...
    for (; i_out_y < dim_im_out_y - padding_y; i_out_y++)
    {
        ...
        for (; i_out_x < dim_im_out_x - padding_x; i_out_x++)
        {
            ...
            for (i_ker_y = i_out_y * stride_y - padding_y; i_ker_y < i_out_y * stride_y - padding_y + dim_kernel_y;
                 i_ker_y++)
            {
                arm_q7_to_q7_reordered_no_shift((q7_t *) Im_in +
